...
module.exports = {
    ...
    addProduct: function (name, category, ingredients, description, picture, prodId) {
        return db.collection('product').doc(prodId).get().then(doc =>{
            if(!doc.exists){
                return db.collection('product').doc(prodId).create({
                    name: name,
                    category: category,
